...
ts::Tensor<T> ts::relu(const ts::Tensor<T> &x) {
	...
	// Get derivative
	// (1 if coef != 0 ; 0 if coef == 0)
	Eigen::Array<T, Eigen::Dynamic, Eigen::Dynamic> dx = res;
	for(unsigned i=0; i<dx.rows(); i++) {
		for(unsigned j=0; j<dx.rows(); j++) {
			dx(i,j) =  (dx(i,j) != 0) ? 1.0 / maxCoeff : 0;
		}
	}
